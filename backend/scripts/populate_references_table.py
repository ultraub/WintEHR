...
This script retroactively populates the references table for resources that were
created before reference extraction was implemented or where it failed.
"""
        ...
        async def process_value(value: Any, path: str):
            ...
            if isinstance(value, dict):
                ...
                if "reference" in value and isinstance(value["reference"], str):
                    ref_parts = value["reference"].split("/")
                    if len(ref_parts) == 2:
                        target_type, target_id = ref_parts
                        
                        # Handle urn:uuid references
                        if target_id.startswith("urn:uuid:"):
                            target_id = target_id[9:]  # Remove urn:uuid: prefix
