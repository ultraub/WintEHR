"""
CDS Hooks business logic, extracted from the 1,777-line cds_hooks_router
(docs/ARCHITECTURE_DEBT.md opportunity #5, third of four god routers).

CDSHooksService takes the request-scoped AsyncSession plus one injected
HAPI client (replacing three per-handler constructions); the registry /
orchestrator / SAMPLE_HOOKS module singletons stay module-level here, as
they were in the router. Method bodies are moved verbatim — including
raising fastapi.HTTPException directly, the same documented fidelity
compromise as pharmacy and orders.
"""
...
class CDSHooksService:
    """CDS Hooks operations: discovery, execution, feedback, service CRUD."""

    def __init__(self, db: AsyncSession, hapi_client: Optional[HAPIFHIRClient] = None):
        self.db = db
        if hapi_client is None:
            # Resolve via module attribute at construction time (one service
            # per request), not the import-time binding: the router's handlers
            # historically did a function-local re-import for exactly this
            # reason, and the test suite patches
            # services.hapi_fhir_client.HAPIFHIRClient at the source.
            from services import hapi_fhir_client as _hapi_module
            hapi_client = _hapi_module.HAPIFHIRClient()
        self.hapi = hapi_client

    async def discover_services(self, *, service_origin: Optional[str] = None):
        """CDS Hooks discovery endpoint - v3.0 architecture (moved verbatim from the router)."""
    ...
    async def execute_service(self, *, service_id: str, request: CDSHookRequest):
        """Execute a specific CDS service - v3.0 architecture (moved verbatim from the router)."""
        ...
        try:
            ...
            if registered_service:
                ...
                if service_origin == "external":
                    ...
                    query = text("""
                        ...
                        LIMIT 1
                    """)
                    result = await self.db.execute(query, {"service_id": external_service_id})
                    ...
                    provider = RemoteServiceProvider(self.db)
                    ...
                    if is_cql_service_type(visual_config.service_type):
                        ...
                        provider = CQLBackedServiceProvider(self.db)
                        response = await provider.execute(
                            plan_definition,
                            request,
                            service_metadata={
            ...
            await log_service_failure(
                self.db, service_id=service_id, request_context=request.context,
            ...
            return CDSHookResponse(cards=[])
    async def provide_feedback(self, *, service_id: str, feedback: FeedbackRequest):
        """Provide feedback on CDS service recommendations - CDS Hooks v2.0 compliant (moved verbatim from the router)."""
        try:
            ...
            manager = await get_persistence_manager(self.db)
            ...
            for feedback_item in feedback.feedback:
                ...
                feedback_id = await process_cds_feedback(self.db, feedback_data)
                ...
                try:
                    summaries = await _execute_accepted_suggestion_actions(
                        self.db, service_id=service_id, feedback_item_data=feedback_data,
                    )
    ...
    async def get_feedback_analytics(self, *, service_id: str, days: int = 30):
        """Get analytics for a specific CDS service (moved verbatim from the router)."""
        try:
            ...
            # Get analytics
            analytics = await get_service_analytics(self.db, service_id, days)
            ...
            raise HTTPException(status_code=500, detail="Failed to retrieve analytics")
    async def get_global_analytics(self, *, days: int = 30):
        """Get analytics summary for all CDS services (moved verbatim from the router)."""
        try:
            feedback_manager = await get_feedback_manager(self.db)
            ...
            raise HTTPException(status_code=500, detail="Failed to retrieve analytics summary")
    async def analyze_prefetch_patterns(self, *, service_id: str, days: int = 30):
        """Analyze prefetch patterns for optimization (moved verbatim from the router)."""
        try:
            # Verify the service exists
            manager = await get_persistence_manager(self.db)
            hook_config = await manager.get_hook(service_id)
            ...
            engine = await get_prefetch_engine(self.db)
            ...
            raise HTTPException(status_code=500, detail="Failed to analyze prefetch patterns")
    async def diagnose_service(self, *, service_id: str):
        """Read-only triage for a CDS service that isn't producing cards. (moved verbatim from the router)."""
        from sqlalchemy import select
        ...
        return report
    async def list_services(self, *, hook_type: Optional[str] = None, enabled_only: bool = True):
        """List all CDS services. (moved verbatim from the router)."""
        try:
            manager = await get_persistence_manager(self.db)
        ...
        try:
            ...
            vresult = await self.db.execute(vquery)
        ...
        return merged
    async def create_service(self, *, hook_config: HookConfiguration):
        """Create a new CDS service (moved verbatim from the router)."""
        try:
            manager = await get_persistence_manager(self.db)
            ...
            raise HTTPException(status_code=500, detail="Failed to create hook")
    async def backup_services(self):
        """Create a backup of all service configurations (moved verbatim from the router)."""
        try:
            manager = await get_persistence_manager(self.db)
            ...
            raise HTTPException(status_code=500, detail="Failed to create backup")
    async def restore_services(self, *, backup_data: Dict[str, Any]):
        """Restore services from backup data (moved verbatim from the router)."""
        try:
            manager = await get_persistence_manager(self.db)
            ...
            raise HTTPException(status_code=500, detail="Failed to restore services")
    async def sync_sample_services(self):
        """Sync sample services to database (moved verbatim from the router)."""
        try:
            await save_sample_hooks_to_database(self.db, SAMPLE_HOOKS)
            db_hooks = await load_hooks_from_database(self.db)
    ...
    async def get_service(self, *, service_id: str):
        """Get a specific CDS service (moved verbatim from the router)."""
        try:
            manager = await get_persistence_manager(self.db)
            hook_config = await manager.get_hook(service_id)
            if not hook_config:
            ...
            raise HTTPException(status_code=500, detail="Failed to retrieve service")
    async def update_service(self, *, service_id: str, hook_config: HookConfiguration):
        """Update a CDS service (moved verbatim from the router)."""
        try:
            manager = await get_persistence_manager(self.db)
    ...
    async def delete_service(self, *, service_id: str):
        """Delete a CDS service (moved verbatim from the router)."""
        try:
            manager = await get_persistence_manager(self.db)
            ...
            raise HTTPException(status_code=500, detail="Failed to delete service")
    async def toggle_service(self, *, service_id: str, enabled: bool):
        """Enable or disable a CDS service (moved verbatim from the router)."""
        try:
            manager = await get_persistence_manager(self.db)
            ...
            raise HTTPException(status_code=500, detail="Failed to toggle service")
    async def list_registry_services(self):
        """List all services registered in the service registry (moved verbatim from the router)."""
    ...
    async def get_registry_service(self, *, service_id: str):
        """Get details of a specific service from the registry (moved verbatim from the router)."""
    ...
    async def health_check(self):
        """CDS subsystem health/diagnostics (DB hooks, registry counts). (moved verbatim from the router)."""
        try:
...
def get_cds_hooks_service(db: AsyncSession = Depends(get_db_session)) -> CDSHooksService:
    """FastAPI dependency — one service per request, sharing the request's DB session."""
    return CDSHooksService(db=db)
