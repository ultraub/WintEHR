...
const LabValueConditionBuilder = ({ condition, onChange, onRemove }) => {
  ...
  // Initialize from existing condition and load dynamic labs
  useEffect(() => {
    // Load initial dynamic catalog without search filter
    const loadInitialLabs = async () => {
      ...
      try {
        ...
        // If we have an existing condition, try to find and set the lab
        if (condition.labTest) {
          const lab = formatted.find(l => l.code === condition.labTest);
          if (lab) {
            setSelectedLab(lab);
          }
        }
  ...
  const searchLabTests = async (query) => {
    ...
    setSearching(true);
    try {
      // Search dynamic lab catalog with query
      const dynamicLabs = await cdsClinicalDataService.getLabCatalog(query, null, 50);
      const formatted = dynamicLabs.map(lab => ({
        code: lab.loinc_code,
        display: lab.display,
        unit: lab.reference_range?.unit || '',
  ...
  const getReferenceRange = () => {
    if (!selectedLab || !selectedLab.reference_range) return null;
    
    // Use dynamic reference range from actual patient data
    return {
      low: selectedLab.reference_range.low,
      high: selectedLab.reference_range.high,
          ...
          <Grid item xs={12}>
            <Alert severity="info" icon={<InfoIcon />}>
              <Typography variant="caption">
                ...
                <> | Based on {selectedLab.source === 'dynamic' ? 'patient data' : 'reference values'}</>
              </Typography>
            </Alert>
          </Grid>
        )}
