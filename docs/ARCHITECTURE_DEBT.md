...
| **4** | **Backend registration/routing hardening.** Per-router try/except; health endpoint reporting failures; one prefix convention; reconcile the three proxy configs. | **mostly DONE** — flat per-router `ROUTERS` list (one failure = one router down, not twelve), `/api/health` reports `FAILED_ROUTERS` (B3 fixed), dead `api/system/health.py` removed, 4 tests; prefix convention normalized to full-prefix-in-file (OpenAPI-diff-proven zero URL changes; deliberate exceptions: `/fhir`, SMART spec paths). **Remaining**: the two escapees (`/clinical/notes`, `/dicom` — public-URL changes) + proxy-config reconciliation (incl. B6 `/ws`). |
