...
| B2 | ~~Keyboard tab lists have 10 entries vs registry's 12~~ **Fixed** — shortcuts and cycle order derive from `clinicalTabRegistry`; ctrl+Tab cycles all 12 tabs, digit keys map to strip position, help dialog derives too; 3 tests | `hooks/ui/useKeyboardNavigation.js`, `registryCoverage.test.js` |
...
| **2** | **One frontend data-access path.** Migrate callers to the existing context/fhirClient stack — not a new layer (two previous new-layer attempts died unadopted). | **in progress** — transports consolidated to exactly two (`fhirClient` for FHIR, `api.js` for backend): `cdsClinicalDataService` off its private axios, `enhancedOrderSearch` off raw fetch (FHIR search now rides fhirClient's cache/dedup/retry; its private search cache dropped); the purge already removed the other two raw-fetch services and the contradictory `clientConfig.ts`. Bundle normalization unified: `core/fhir/utils/bundleUtils.extractBundleResources` (handles SearchResult/Bundle/wrapped/null + the #281-class resourceType filter) replaced 41 hand-rolled `entry→resource` mappings across 17 files, incl. the context's own standardizeResponse; FHIR *List* resource `.entry` sites were adjudicated as NOT bundles and deliberately left. Cache collapse done as a RULE plus targeted fixes (recorded in `src/services/CLAUDE.md` §Caching rules): raw FHIR responses are fhirClient's to cache, and both module-level forever-caches of FHIR data were removed — `useMedicationResolver`'s never-evicted medication Map (stale meds + stuck failure-nulls) and `DiagnosisPicker`'s per-patient condition Map (new problems invisible until page reload); api.js-transport caches and component-lifetime/derived-value memoization stay by design. Tab migration: Results (3 searches) and Summary (3 searches) now flow through `searchResources` — shared store, dedup, one cache; ImagingTab is a DOCUMENTED exception (its `_include`'d Endpoints would be dropped by the #281 type filter — migrate when `searchResources` returns `{resources, included}` separately). Context callback identities STABILIZED: the six leaf offenders (`getCachedData`, `cleanupOldResources`, `setResources`, `addResource`, `clearCache`, `fetchPatientBundle`) now use `useStableCallback`, cascading stable identities to every action built on them; render-time READERS deliberately keep reactive identities (policy comment at the site). The Results/Summary dep-array exclusions are removed — honest deps are loop-safe now. Include-support closed and ImagingTab migrated — no tab issues raw FHIR searches anymore. Pharmacy side-services ADJUDICATED done-by-substrate: all four already ride fhirClient, and their Maps are workflow state (subscriptions, List working sets), not response caches — legitimate feature-layer services under the codified rules. **#2 COMPLETE**: two transports, one bundle normalization, one caching policy, stable context identities, every tab through the context. Revisit pharmacy only if behavior bugs surface. |
| **3** | **Resource-type registry.** One frontend module (icon, color, label, priority tier, sort params, export columns per type) and one backend `REFERENCE_FIELDS` source of truth; point the 14 catalogs and 7 priority lists at it. Drops "add a resource type" from 25–30 touch points to a handful. | **in progress** — `core/fhir/resourceRegistry.js` owns tiers/membership/labels/HAPI-sort-names; all surviving priority lists + sort maps migrated (2dd9f23a); hook-type lists unified (B4). B5 done (`api/fhir/reference_fields.py`). Display metadata: **boundary decision** — the registry owns semantics; themes/features own presentation (colors stay in `categoricalAccents`, icons in their features), with `registryCoverage.test.js` as the drift alarm binding them to registry membership. Explorer/Timeline per-type configs stay feature-local by the same rule. |
...
| **5** | **Service extraction for the four god routers** (pharmacy, orders, cds_hooks, visual_builder — ~6,400 lines of inline logic). After #4, one router at a time, adding tests per extracted service. | **in progress** — **pharmacy DONE**: 1,526-line router split into `models.py` (8 models) + `service.py` (~1,300 lines, ONE injected HAPI client replacing 12 per-handler constructions) + thin `pharmacy_router.py` stubs; the dispense/MAR signing gates moved with the logic, their tests rewritten to inject a mock client (no dotted-path patches), and both gates mutation-proven (sabotage→red, restore→green); extraction surfaced B12. **orders DONE**: 1,567-line router split the same way (14 models, 11 methods, 10 client constructions collapsed to one); the router had ZERO prior test coverage — the extraction added the first tests for the CPOE allergy-blocking gate (high alert blocks unless overridden; override stamped as extensions) and the discontinue status gate (active/draft only; MedicationRequest→stopped+statusReason, ServiceRequest→revoked+note), both mutation-proven. **cds_hooks DONE**: 1,777-line router split into `api/cds_hooks/service.py` (CDSHooksService: discovery/execution/feedback/CRUD, request-scoped db + injected HAPI client resolved late so source-module test patches still bind) + 310-line stub router; the four `*_alias` routes now delegate to the same service method (orders' NameError class eliminated by construction); two hidden dependents repointed (`actions/router.py` imported get_persistence_manager THROUGH the router; `cds_studio/service.py` called the execute_service handler). **visual_builder DONE**: 1,539-line router split into `visual_builder_service.py` + stub router; first tests added for `_validate_cql_valueset_urls` (save-time rejection of unresolvable ValueSet URLs) and `_data_requirements_to_prefetch`. **#5 COMPLETE** — all four god routers are models/service/thin-stubs with injected clients. Documented compromise in all services: methods raise `HTTPException` directly (bodies moved verbatim for behavior fidelity); a domain-exception pass is possible follow-up. |
...
| B13 | ~~`GET /api/clinical/orders/` (and `/active`) 500'd since inception — B12's twin: one shared `_sort=-authored` sent to both resource types; ServiceRequest accepts it, MedicationRequest's parameter is `authoredon`, HAPI 400s, endpoint 500s~~ **Fixed** — per-type sort inside the query loop, with a test asserting both outgoing sorts. Same discovery path as B12: green suite, dead endpoint, caught only by curling the deployed box | `api/clinical/orders/service.py` |
| B15 | ~~MAR: recording a dose didn't flip its grid cell when charted >60 min from the scheduled slot — the record endpoint stamped the cell's scheduled time on a `scheduled-dose-time` extension, but the matcher never read it and matched only on effective-time proximity; the orphaned admin fell into `unscheduled_admins`, which the UI doesn't render, so the nurse's action looked like a no-op~~ **Fixed** — two-pass match: the stamp is the authoritative link (exact, steal-proof), proximity remains the fallback for admins recorded outside the MAR. Found live on wintehrdev (two orphaned demo-nurse admins, retroactively matched by the fix) | `api/clinical/administration/service.py` |
| B16 | ~~MAR left rail garbled: fixed-height row let long `dose_text` wrap with no overflow clip — med name pushed out of view under the sticky header, dose fragments bled across rows~~ **Fixed** — rail clips, dose text single-line ellipsized with tooltip, indication chip width-capped | `AdministrationRecord/MedRowHeader.jsx` |
| B17 | ~~DICOM viewer dead on any stock dcm4chee deployment: the compose-default `DICOM_QIDO_URL` lacked a trailing slash, so `urljoin(base, "studies")` REPLACED the `/rs` segment (404 → 503 → viewer silently blank); the default `DICOM_WADO_URL` pointed at the legacy WADO-URI endpoint though the service builds WADO-RS paths~~ **Fixed** — service normalizes both URLs into urljoin-safe bases; compose defaults point both at the RS root; regression tests pin it. wintehrdev never hit it (its .env spelled the URL with a slash); found restoring imaging on eastus2. Companion UX wart also fixed: the viewer's loading/error branches rendered bare in-flow elements (only the success branch had the fixed overlay), so a metadata failure painted its Alert off-viewport — now all three states share the overlay, the error names the server's detail, and Try again / Close are offered; pinned by DICOMViewer.error.test.jsx | `api/dicom/service.py`, `docker-compose.yml`, `imaging/DICOMViewer.js` |
| B14 | **OPEN — design decision needed.** Training-mode sessions live in an in-process class dict (`AuthService.training_sessions`) but prod runs uvicorn `--workers 4` — a login lands on one worker, so any Bearer-authenticated call 401s ~75% of the time. The UI never notices only because its clinical reads/writes ride the `/fhir` proxy and most backend endpoints skip auth; any direct API consumer (curl, scripts, external SMART apps using session tokens) hits the roulette. Fix direction: store training sessions in Redis (`REDIS_URL` already provisioned and documented as "session caching" — the code never used it for that) | `api/auth/service.py` |
