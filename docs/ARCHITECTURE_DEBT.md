...
| **4** | **Backend registration/routing hardening.** Per-router try/except; health endpoint reporting failures; one prefix convention; reconcile the three proxy configs. | **registration half DONE** — flat per-router `ROUTERS` list (one failure = one router down, not twelve), `/api/health` reports `FAILED_ROUTERS` (B3 fixed with it), dead `api/system/health.py` removed, 4 tests. **Remaining**: prefix normalization (6 conventions → 1) + proxy-config reconciliation (incl. B6 `/ws`). |
